  ...
  constructor() {
    this.cfg = {
      replicate: {
        ...
        key: "r8_UrqnzTN4bmeCPQoinYPVmYCz5pVoijH2AKtOE",
        models: {
  ...
  async genRep(prompt, imageUrl, model, opts) {
    ...
    try {
      ...
      const {
        ...
        headers: {
          Authorization: `Token ${this.cfg.replicate.key}`,
          "Content-Type": "application/json"
        }
      });
      const pollUrl = data?.urls?.get || "";
      console.log(`[REP] Job created: ${data?.id || "unknown"}`);
      return await this.pollRep(pollUrl);
    } catch (e) {
      console.error(`[REP] Error: ${e?.response?.data?.detail || e?.message || e}`);
      throw e;
    }
  }
  async pollRep(url) {
