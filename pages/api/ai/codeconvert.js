...
class CodeConvert {
  constructor() {
    this.cfg = {
      ...
      // List bahasa asli
      langs: [
        "C++", "Golang", "Java", "JavaScript", "Python", "R", "C", "Csharp", "Julia", "Perl", 
        "Matlab", "Kotlin", "PHP", "Ruby", "Rust", "TypeScript", "Lua", "SAS", "Fortran", 
        "Lisp", "Scala", "Assembly", "ActionScript", "Clojure", "CoffeeScript", "Dart", 
        "COBOL", "Elixir", "Groovy", "Erlang", "Haskell", "Pascal", "Swift", "Scheme", 
        "Racket", "OCaml", "Elm", "Haxe", "Crystal", "Fsharp", "Tcl", "VB.NET", "Objective_C", 
        "Ada", "Vala", "PySpark", "SQL", "PostgreSQL", "MySQL", "MongoDB", "CQL", "Redis", 
        "Elasticsearch", "VB6", "VBA", "VBScript", "PowerShell", "Bash", "Delphi", "Zig", 
        "Carbon", "Nim", "Grain", "Gleam", "Wren"
      ],
      hdr: {
        accept: "*/*",
      ...
      map: {
        generate: { code: "inputText", lang: "inputLang" },
        convert: { code: "inputCodeText", lang: "inputLang", target: "outputLang", instruction: "customInstruction" },
        explain: { code: "inputCodeText", lang: "inputLang", instruction: "customInstruction" },
        remove: { code: "inputCodeText", lang: "inputLang" }
      }
    };
  }
  ...
  validInput(mode, data) {
    ...
    if (data?.lang) {
      ...
      data.lang = correctedLang; // Mutasi ke format yang benar
    }

    // 3. Validasi Target Lang
    if (data?.target) {
      ...
      data.target = correctedTarget; // Mutasi ke format yang benar
    }
  ...
  parseResult(data) {
    ...
    const { outputCodeText, outputText, ...info } = data || {};
    return {
      result: result,
      ...info
    };
  ...
  async chat({ mode, ...rest }) {
    ...
    if (!validation.valid) {
      const err = {
        error: true,
        // Message sekarang berbentuk Object
    ...
    try {
      ...
      return {
        error: false,
        ...parsed,
        // Message sukses juga kita buat format object standar
        message: {
            text: "Success",
            mode: mode
        }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Response JSON otomatis memuat struktur message yang sudah diubah di atas
